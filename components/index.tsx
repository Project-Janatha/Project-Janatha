...
export function SecondaryButton({ children, ...props }) {
  return (
    <Pressable className="border border-borderColor bg-transparent text-color px-4 py-3 rounded-lg active:bg-gray-4" {...props}>
...
export function GhostButton({ children, ...props }) {
  return (
    <Pressable className="bg-transparent text-color px-4 py-3 rounded-lg active:bg-gray-200" {...props}>
      <Text className="text-color text-base text-center">{children}</Text>
    </Pressable>
  );
}
